"""Super-admin kitchen workspace — WhatsApp travels with the kitchen."""
...
ADMIN_EMAIL = "admin-kitchen-int@test.ckac"
...
def _seed() -> tuple[uuid.UUID, str]:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            VALUES (%s::uuid, %s, 'hash', 'Kitchen Int Admin', 'superadmin', true)
        ...
        cur.execute(
            ...
            VALUES (%s::uuid, %s, 'WA Owner', 'starter', 'trial')
            """,
        ...
        cur.execute(
            ...
            VALUES (
                %s::uuid, %s::uuid, %s, 'WA Test Kitchen',
                ST_SetSRID(ST_MakePoint(73.8958, 18.5362), 4326)::geography,
                'active', 'Pune'
            )
            """,
            (str(kitchen_id), str(owner_id), f"CKW{owner_id.hex[:4].upper()}"),
        )
    conn.close()
...
@pytest.mark.asyncio
async def test_admin_kitchen_whatsapp_crud_and_detail(client: AsyncClient):
    ...
    assert body["payment_gateway_configured"] is False
    assert "platform_secrets_note" in body

    put = await client.put(
        f"/api/v1/admin/kitchens/{kitchen_id}/whatsapp-integration",
        headers=headers,
        json={
            "whatsapp_phone_id": "10998877665544",
            "whatsapp_display_phone": "+919876543210",
        },
    )
    assert put.status_code == 200, put.text
    assert put.json()["connected"] is True
    assert put.json()["whatsapp_phone_id"] == "10998877665544"

    listed = await client.get("/api/v1/admin/kitchens", headers=headers)
    assert listed.status_code == 200
    row = next(k for k in listed.json() if k["id"] == str(kitchen_id))
    assert row["whatsapp_connected"] is True
    ...
    assert clear.json()["connected"] is False
