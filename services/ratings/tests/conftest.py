"""Ratings service tests — TDD (Sprint 11)."""
...
async def _flush_rating_streams() -> None:
    import redis.asyncio as redis

    client = redis.from_url(os.environ["REDIS_URL"], decode_responses=True)
    try:
        await client.delete("ckac:ratings:rating", "ckac:ratings:dish")
    finally:
        await client.aclose()
...
def _seed_ratings_ctx(*, order_status: str = "delivered") -> dict:
    owner_id = uuid.uuid4()
    ...
    code = f"CKR{owner_id.hex[:4].upper()}"
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            """,
            (str(owner_id), phone, "Ratings Owner"),
        )
        cur.execute(
            """
            ...
            VALUES (
                %s::uuid, %s::uuid, %s, 'Rating Kitchen',
        ...
        cur.execute(
            ...
            VALUES (%s::uuid, %s, 'Rating Customer', 'active')
            """,
        ...
        cur.execute(
            ...
            VALUES (
                %s::uuid, %s::uuid, 'BILL-20260712-0200', %s, %s, 'pwa',
                'delivery', 'cod', %s, 'Rating Customer', 298.00, 0, 298.00
            )
            """,
            (
                str(order_id),
                str(kitchen_id),
                f"{code}-BILL-20260712-0200",
                order_status,
                customer_phone,
            ),
        )
        cur.execute(
            ...
            VALUES (%s::uuid, %s::uuid, %s::uuid, 'Dal Tadka', 2, 149.00)
            """,
...
@pytest.fixture
def preparing_ctx() -> dict:
    return _seed_ratings_ctx(order_status="preparing")


@pytest.fixture
def ratings_ctx() -> dict:
    return _seed_ratings_ctx()
