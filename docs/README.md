...
| **[CKAC-COMPLETE-GUIDE.md](./CKAC-COMPLETE-GUIDE.md)** | **Master guide v3.2.8 — modules through P56 (Today OS, bounded order list, store apps, sales onboard), flows §17.9–17.12, UI Catalog, OpenAPI** | CEO, CPO, CTO, DBA, QA, Investors, AI agents |
| **[CKAC-COMPLETE-GUIDE.pdf](./CKAC-COMPLETE-GUIDE.pdf)** | Complete Executive Guide PDF v3.2.8 (portrait; UI Catalog figures) | CEO, CPO, CTO, Investors |
...
| **[TESTER-INSTRUCTION-PACK.md](./TESTER-INSTRUCTION-PACK.md)** | **Tester book v1.2** — numbered UI + Swagger/API steps for portal, customer, kitchen, admin, **sales**, store apps | QA testers, release sign-off |
...
# QA instruction pack (short Must/Should checklist)
python scripts/generate_qa_instruction_pdf.py

# Tester instruction pack (numbered UI + API steps)
python scripts/generate_tester_instruction_pdf.py
```

## Quick Stats

- **S1–S18 + P19–P56 shipped** — track progress in [ADVANCEMENT-TRACKER.md](./ADVANCEMENT-TRACKER.md); encyclopedia [Complete Guide v3.2.8](./CKAC-COMPLETE-GUIDE.md)
...
- **API:** Aggregated OpenAPI at gateway `/openapi.json` / `/docs` / `/redoc`, portal `/openapi` (`/api-docs`) — see [`API.md`](./API.md) §1.1–1.2 (Authorize via OAuth2Password `POST /api/v1/auth/token` or HTTPBearer; public ops have no padlock). Super Admin Sign in shows username/password when the login-hint reveal gate is on.
