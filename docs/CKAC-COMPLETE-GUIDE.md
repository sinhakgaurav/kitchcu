...
**Six-month demo history:** `python scripts/seed-bulk-data.py` defaults to `CKAC_BULK_MONTHS=6` and stamps IST service-hour orders across ~183 days (age-aware status, diner pool, AOV ~₹450–550). Owner Reports expose a 6-month range. Weekly Saturday cron fills 21 orders/kitchen across the trailing 7 days. See [`ADVANCEMENT-TRACKER.md`](./ADVANCEMENT-TRACKER.md) § Six-month trading history.
