...
export function CrmPage() {
  ...
  return (
    <div className="owner-page">
      <header className="owner-page__head">
        <div>
          ...
          <p className="owner-page__code">
            Spend, favorites & order patterns — synced from your orders
          </p>
        </div>
      ...
      {syncedAt && (
        <p className="owner-page__meta">Last synced {new Date(syncedAt).toLocaleString("en-IN")}</p>
      )}
      {error && <p className="form-error">{error}</p>}
      {loading ? (
        <div className="app-loading">Loading customers...</div>
      ) : customers.length === 0 ? (
        <p className="owner-empty">No customers yet — orders with phone numbers appear here.</p>
      ) : (
              ...
              {customers.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>
                    ...
                    />
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => saveTags(c)}>
                      Save
                    </button>
                  </td>
                </tr>
